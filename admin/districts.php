...
if (isset($_POST['delete'])) {
    $id = $_POST['id'];
    $stmt = $pdo->prepare("DELETE FROM districts WHERE district_id = ?");
    $stmt->execute([$id]);
}

$allowed_sorts = ['district_id', 'district_name'];
$sort = in_array($sort, $allowed_sorts) ? $sort : 'district_id';

$count_stmt = $pdo->prepare("SELECT COUNT(*) FROM districts WHERE district_name LIKE ?");
$count_stmt->execute(["%$search%"]);
$total_items = $count_stmt->fetchColumn();
$total_pages = ceil($total_items / $items_per_page);

$stmt = $pdo->prepare("SELECT * FROM districts WHERE district_name LIKE ? ORDER BY $sort $order LIMIT ? OFFSET ?");
...
<body>
    ...
    <div class="container">
        ...
        <table>
            <tr>
                <th><a href="?sort=district_id&order=<?php echo $order === 'ASC' ? 'DESC' : 'ASC'; ?>">ID</a></th>
                <th><a href="?sort=district_name&order=<?php echo $order === 'ASC' ? 'DESC' : 'ASC'; ?>">Name</a></th>
                <th>Actions</th>
            </tr>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <form method="POST" style="display:inline;">
                            <input type="hidden" name="id" value="<?php echo $district['district_id']; ?>">
